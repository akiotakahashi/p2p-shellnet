	...
	internal class PipeConnection : IDisposable {
		...
		private void Connect() {
			while (true) {
				...
				if(_handle != PipeNative.INVALID_HANDLE_VALUE) {
					return;
				} else if(PipeNative.GetLastError() != PipeNative.ERROR_PIPE_BUSY) {
					throw new PipeIOException("Could not open pipe: " + _pipeName);
				} else if(!PipeNative.WaitNamedPipe(_pipeName, 20000)) {
		...
		private void Create(IntPtr pipeSecurityDescriptor) {
			_handle = PipeNative.CreateNamedPipe(_pipeName
				...
				, PipeNative.PIPE_TYPE_BYTE
				| PipeNative.PIPE_READMODE_BYTE
				| PipeNative.PIPE_WAIT
		...
		public void Write(Stream str) {
			...
			if(str==null) {
				...
				while (len != 0) {
					...
					if (len > 0) {
						_writer.Write(buffer,0,(int)len);
						/*
						try {
							Console.Error.Write(System.Text.Encoding.UTF8.GetString(buffer,0,(int)len));
						} catch {
						}
						*/
					}
		...
		public void EndWriteMessage() {
			...
			if(fOk) {
				fOk = PipeNative.WriteFile(_handle, 
					...
					0);
				if(!fOk) {
					throw new PipeIOException("Error writing stream to pipe " + _handle + ": error " + PipeNative.GetLastError());
				}
			} else {
				throw new PipeIOException("Error writing length to pipe " + _handle + ": error " + PipeNative.GetLastError());
			}
		...
		public void BeginReadMessage() {
			...
			if(fOk) {
				...
				fOk = PipeNative.ReadFile(_handle,
					...
					0);                               
				if(!fOk) {
					throw new PipeIOException("Error reading stream from pipe " + _handle + ": error " + PipeNative.GetLastError());
				}
			} else {
				throw new PipeIOException("Error reading length from pipe " + _handle + ": error " + PipeNative.GetLastError());
			}
